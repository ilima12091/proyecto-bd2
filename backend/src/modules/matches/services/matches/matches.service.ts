...
export class MatchesService {
  ...
  async create(createMatchDTO: CreateMatchDTO) {
    const insertData = await this.matchesRepository.create(createMatchDTO);

    return insertData;
  }

  async update(id: number, createMatchDTO: CreateMatchDTO) {
    return await this.matchesRepository.update(id, createMatchDTO);
  }
  ...
  async insertResult(matchId: number, localGoals: number, awayGoals: number) {
